...
igraph_error_t igraph_weighted_sparsemat(igraph_t *graph, const igraph_sparsemat_t *A,
    ...
    igraph_attribute_record_list_t attr_list;
    ...
    IGRAPH_CHECK(igraph_attribute_record_list_init(&attr_list, 1));
    IGRAPH_FINALLY(igraph_attribute_record_list_destroy, &attr_list);
    ...
    attr_rec = igraph_attribute_record_list_get_ptr(&attr_list, 0);
    ...
    if (igraph_vector_int_size(&edges) > 0) {
        IGRAPH_CHECK(igraph_add_edges(graph, &edges, &attr_list));
    }
    IGRAPH_FINALLY_CLEAN(1);
    ...
    igraph_attribute_record_list_destroy(&attr_list);
