...
```bash
git clean -fdx src
UserNM=true _R_SHLIB_STRIP_=true R CMD INSTALL . --no-byte-compile 2>&1 | tail -20
Rscript -e 'testthat::test_local(reporter = "check", stop_on_failure = FALSE)' 2>&1 | tail -40
```

The two env vars on the `R CMD INSTALL` line each skip an expensive post-build
step we don't need for vendoring:
`UserNM=true` skips an `nm`-driven symbol scan, and `_R_SHLIB_STRIP_=true`
skips stripping the shared library. The same prefix is used in
`.github/workflows/copilot-setup-steps.yaml`; carry it through every
`R CMD INSTALL` invocation in this workflow.
...
Stimulus regeneration fails), the commit was **not** created. Almost always
this means either (a) a `patch/` file needs adjusting for an upstream API
change, or (b) a Stimulus definition needs to be updated to keep the
autogenerated bridge buildable.

**Don't re-run the vendor script** to recover — it isn't designed to resume
cleanly from a half-imported state, and a second run can clobber in-flight
work or simply fail the same way. Instead, finish the import by hand from
where the script left off:

1. Add or edit the relevant `patch/00NN-*.patch` (and/or
   `tools/stimulus/*.yaml`) and apply it directly to the working tree:
   `patch -p1 -i patch/00NN-*.patch` for any new/changed C patch. The
   patch lives in `patch/` so the **next** vendoring pass (whether
   later in this loop or a future one) will pick it up automatically when
   the script re-clones cigraph and re-applies all patches.
2. Re-run the remaining vendor steps manually for **this** commit only:
   ```bash
   make -f Makefile-cigraph
   R -q -e 'cpp11::cpp_register()'
   ```
3. Bump the fifth `Version:` component in `DESCRIPTION` (the same way
   `scripts/vendor-one.sh` does), stage everything, and create the
   `vendor: Update vendored sources to igraph/igraph@<sha>` commit by
   hand. The contents of the message body should match what
   `scripts/vendor-one.sh` would have written (date + first-parent log of
   the range being imported); see the script for the exact template.

From here on, the commit exists and the rest of Phase 2 proceeds normally.
...
```bash
git clean -fdx src
UserNM=true _R_SHLIB_STRIP_=true R CMD INSTALL . --no-byte-compile 2>&1 | tail -10
Rscript -e 'testthat::test_local(reporter = "check", stop_on_failure = FALSE)' 2>&1 | tail -40
```
